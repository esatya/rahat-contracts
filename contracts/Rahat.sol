...
contract Rahat is AccessControl, ERC1155Holder, Multicall {
    ...
    bytes32 public constant GET_TOKENS_FROM_CLAIM =
        keccak256("getTokensFromClaim");
    ...
    function getTotalERC1155Issued(
        uint256 _phone
    )
        public
        view
    ...
    //TODO replace with multicall
    function getTotalERC1155Balance(
        uint256 _phone
    )
        public
        view
    ...
    //TODO replace with multicall
    function getTotalERC1155IssuedBy(
        address _address
    )
        public
        view
    ...
    function suspendBeneficiary(
        uint256 _phone,
        bytes32 _projectId
    ) public OnlyAdmin IsBeneficiary(_phone) {
        //  bytes32 _benId = findHash(_phone);
    ...
    function createERC20Claim(
        uint256 _phone,
        uint256 _tokens
    ) public IsBeneficiary(_phone) OnlyVendor {
        bytes32 _benId = findHash(_phone);
        require(
        ...
        emit ClaimedERC20(msg.sender, _phone, _tokens);
    }
    ...
    function createERC1155Claim(
        ...
        ac.amount = _amount;
        ac.date = block.timestamp;
        emit ClaimedERC1155(msg.sender, _phone, _tokenId, _amount);
    }
    ...
    function getERC20FromClaim(
        ...
        require(
            ac.date >= block.timestamp,
            "RAHAT: Claim has already expired."
        );
    ...
    function getERC1155FromClaim(
        ...
        bytes32 otpHash = findHash(_otp);
        require(
            recentERC1155Claims[msg.sender][_benId][_tokenId].otpHash ==
                otpHash,
            "RAHAT: OTP did not match."
        );
