...
contract RahatAgency {
  ...
  mapping(address => mapping(address=>uint256)) public currentProjectAllocation; // Project > DAO > Token (before transfer)
  ...
  function isConfirmed(bytes32 _hash)
      public
      view
  ...
  function allocateToken(address _project, address _dao, uint256 _amount) public OnlyAdmin {
  ...
  function transferToDao(address _project, address _dao) public OnlyAdmin {
    ...
    AgencyToken.mintToken(_dao, _currentDaoBalance);

    totalProjectToken[_project] = totalProjectToken[_project] + _currentDaoBalance;
    totalDaoToken[_dao] = totalDaoToken[_dao] + _currentDaoBalance;
    totalProjectDaoToken[_project][_dao] = totalProjectDaoToken[_project][_dao] + _currentDaoBalance;
  ...
  function transferToken(address _tokenAddress, address _destAddress, uint256 _amount) public OnlyAdmin  {
    IRahatAgencyToken(_tokenAddress).transfer(_destAddress, _amount);
  }

  function transferEther(address payable _destAddress, uint256 _amount) public returns (bool) {
